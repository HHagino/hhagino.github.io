...
| \(I_q^{KE}(X,Y)+I_q^{KE}(JX,JY)=4g_{Pl,q}^{KE}(X,Y)\) | **Derived here using companion exact HBF theorem** | background: [FKMMSV10; BC94; Var24] | Exact once the ambient BLS measurement transport and projective holomorphic determinant line are used. |
...
3. **Ambient antisymmetric many-body field** \(igwedge^N\mathscr L\): the level on which the ordinary position PVM acts and the Slater Born density becomes the DPP.
